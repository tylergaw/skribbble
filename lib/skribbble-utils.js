...
// We need a group to add the shot images to. If a group is selected we'll use
// that, if not a new group will be created. If something other than a group or
// artboard is selected we cannot create images there so this will bail with an alert.
function getShotsGroup () {
  var app = NSApplication.sharedApplication();
  var shotsGroup = null;
  var s = sketch.selection;

  if (s.count() == 0) {
    shotsGroup = sketch.doc.currentPage().addLayerOfType("group");
    shotsGroup.name = "Dribbble shots";
    removeGroupOnCancel = sketch.doc.currentPage();
  }
  else {
    var type = s[0].className();

    if (type == 'MSArtboardGroup') {
      shotsGroup = s[0].addLayerOfType("group");
      shotsGroup.name = "Dribbble shots";
    }
...
function createGrid (opts) {
  ...
  sketch.doc.showMessage('Creating ' + numOfShots + ' shots in "' + opts.shotsGroup.name() + '"...');
